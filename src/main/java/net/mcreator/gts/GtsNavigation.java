...
public class GtsNavigation extends GroundPathNavigation {
	public GtsNavigation(Mob entity, Level world) {
		super(entity, world);
	}
	...
	protected PathFinder createPathFinder(int maxVisitedNodes) {
		...
		return new GtsPathFinder(this.nodeEvaluator, maxVisitedNodes);
	}
			...
			for (int x = x0; x != x1; x += stepx) {
				for (int z = z0; z != z1; z += stepz) {
					for (int y = y0; y != y1; y += stepy) {
						BlockPos currentPos = pos.set(x, y, z);
						BlockState block = this.level.getBlockState(currentPos);
						if (!block.isPathfindable(this.level, currentPos, PathComputationType.LAND))
							return false;
					}
					BlockPathTypes below = this.mob.getPathfindingMalus(BlockPathTypes.WATER) < 0.0F ? BlockPathTypes.WATER : BlockPathTypes.WALKABLE;
					if (below == BlockPathTypes.WATER || below == BlockPathTypes.LAVA || below == BlockPathTypes.OPEN)
						return false;
					float priority = this.mob.getPathfindingMalus(BlockPathTypes.WALKABLE);

...
import net.mcreator.gts.procedures.AraTickUpdateProcedure;
import net.mcreator.gts.procedures.AraSpawnConditionProcedure;
...
public class AraEntity extends Monster implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(AraEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(AraEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(AraEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_passengerSlots = SynchedEntityData.defineId(AraEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<String> DATA_AnimationDecision = SynchedEntityData.defineId(AraEntity.class, EntityDataSerializers.STRING);
	...
	public AraEntity(EntityType<AraEntity> type, Level world) {
		super(type, world);
		xpReward = 50;
		setNoAi(false);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "araspawntexture");
		this.entityData.define(DATA_passengerSlots, 1);
		this.entityData.define(DATA_AnimationDecision, "Ara.Spawn");
	}
	...
	public void baseTick() {
		super.baseTick();
		AraTickUpdateProcedure.execute(this.level(), this);
...
public static void init(SpawnPlacementRegisterEvent event) {
		event.register(GtsModEntities.ARA.get(), SpawnPlacements.Type.ON_GROUND, Heightmap.Types.MOTION_BLOCKING_NO_LEAVES, (entityType, world, reason, pos, random) -> {
			int x = pos.getX();
			int y = pos.getY();
			int z = pos.getZ();
			return AraSpawnConditionProcedure.execute(world, x, y, z);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.getEntityData().get(AraEntity.DATA_AnimationDecision).equals("empty")) {
			...
			event.getController().setAnimation(RawAnimation.begin().thenPlay(this.getEntityData().get(AraEntity.DATA_AnimationDecision)));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 45) {
			this.remove(AraEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
	...
	public Vec3 getPassengerAttachmentPoint(Entity entity, EntityDimensions entityDimensions, float yaw) {
		...
		return this.getPassengers().size() < this.getEntityData().get(AraEntity.DATA_passengerSlots);
	}

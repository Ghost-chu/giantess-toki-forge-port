...
import net.mcreator.gts.procedures.TamedAraTickUpdateProcedure;
import net.mcreator.gts.procedures.TamedAraMobAggroCheckProcedure;
import net.mcreator.gts.procedures.TamedAraAiConditionProcedure;
...
public class TamedAraEntity extends TamableAnimal implements GeoEntity {
	public static final EntityDataAccessor<Boolean> SHOOT = SynchedEntityData.defineId(TamedAraEntity.class, EntityDataSerializers.BOOLEAN);
	public static final EntityDataAccessor<String> ANIMATION = SynchedEntityData.defineId(TamedAraEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<String> TEXTURE = SynchedEntityData.defineId(TamedAraEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Integer> DATA_passengerSlots = SynchedEntityData.defineId(TamedAraEntity.class, EntityDataSerializers.INT);
	public static final EntityDataAccessor<String> DATA_AnimationDecision = SynchedEntityData.defineId(TamedAraEntity.class, EntityDataSerializers.STRING);
	public static final EntityDataAccessor<Boolean> DATA_Sleep = SynchedEntityData.defineId(TamedAraEntity.class, EntityDataSerializers.BOOLEAN);
	...
	public TamedAraEntity(EntityType<TamedAraEntity> type, Level world) {
		super(type, world);
		xpReward = 10;
		setNoAi(false);
	...
	protected void defineSynchedData() {
		...
		this.entityData.define(TEXTURE, "aratexture");
	...
	protected void registerGoals() {
		...
		this.targetSelector.addGoal(1, new HurtByTargetGoal(this) {
			public boolean canUse() {
				double x = TamedAraEntity.this.getX();
				double y = TamedAraEntity.this.getY();
				double z = TamedAraEntity.this.getZ();
				Entity entity = TamedAraEntity.this;
				Level world = TamedAraEntity.this.level();
				return super.canUse() && TamedAraAiConditionProcedure.execute(entity);
			}
			...
			public boolean canContinueToUse() {
				...
				Entity entity = TamedAraEntity.this;
				Level world = TamedAraEntity.this.level();
				return super.canContinueToUse() && TamedAraAiConditionProcedure.execute(entity);
			}
		}.setAlertOthers());
		...
		this.targetSelector.addGoal(4, new NearestAttackableTargetGoal(this, Monster.class, false, false) {
			public boolean canUse() {
				...
				Entity entity = TamedAraEntity.this;
				Level world = TamedAraEntity.this.level();
				return super.canUse() && TamedAraMobAggroCheckProcedure.execute(world, entity);
			}
			...
			public boolean canContinueToUse() {
				...
				Entity entity = TamedAraEntity.this;
				Level world = TamedAraEntity.this.level();
				return super.canContinueToUse() && TamedAraMobAggroCheckProcedure.execute(world, entity);
			}
		});
	...
	public SoundEvent getAmbientSound() {
		if ((this.getHealth() / this.getMaxHealth()) <= 0.5 && this.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() >= 3 && !GtsModVariables.MapVariables.get(this.level()).DisableStomachSound) {
			return BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("gts", "ara_stomach"));
		} else {
	...
	public void baseTick() {
		super.baseTick();
		TamedAraTickUpdateProcedure.execute(this.level(), this);
		this.refreshDimensions();
	}
	public AgeableMob getBreedOffspring(ServerLevel serverWorld, AgeableMob ageable) {
		TamedAraEntity retval = GtsModEntities.TAMED_ARA.get().create(serverWorld);
	...
	private PlayState movementPredicate(AnimationState event) {
		if (this.getEntityData().get(TamedAraEntity.DATA_AnimationDecision).equals("empty")) {
			...
			event.getController().setAnimation(RawAnimation.begin().thenPlay(this.getEntityData().get(TamedAraEntity.DATA_AnimationDecision)));
		}
	...
	protected void tickDeath() {
		...
		if (this.deathTime == 45) {
			this.remove(TamedAraEntity.RemovalReason.KILLED);
			this.dropExperience();
		}
	}
	...
	public Vec3 getPassengerAttachmentPoint(Entity entity, EntityDimensions entityDimensions, float yaw) {
		...
		return this.getPassengers().size() < this.getEntityData().get(TamedAraEntity.DATA_passengerSlots);
	}

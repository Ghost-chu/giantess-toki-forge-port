...
public class TamedTokiKickProcedure {
	public static void execute(LevelAccessor world, Entity sourceentity) {
		...
		if (sourceentity instanceof TamedTokiEntity _datEntSetS)
			_datEntSetS.getEntityData().set(TamedTokiEntity.DATA_AnimationDecision, "Toki.kick");
		...
		GtsMod.queueServerWork((int) 12.5, () -> {
			if (sourceentity.isAlive()) {
							...
							if (!entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("gts", "giantessfaction")))
								...
								if (entityiterator instanceof LivingEntity _livEnt23 && _livEnt23.isBlocking()) {
									if (entityiterator instanceof LivingEntity _entity) {
										...
										_entity.hurt(new DamageSource(_dmgsource.typeHolder(), _dmgsource.getEntity(), _dmgsource.getDirectEntity()) {
											...
											public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
												String _translatekey = (entityiterator.getDisplayName().getString() + " tried and failed to block Toki's kick, and died from embarrassment");
										...
										}, (float) (1 * (sourceentity instanceof LivingEntity _livingEntity24 && _livingEntity24.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity24.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0)));
									}
									...
									entityiterator.hurt((sourceentity instanceof LivingEntity _attacker ? _attacker.damageSources().mobAttack(_attacker) : sourceentity.damageSources().generic()),
											(float) (2.5 * (sourceentity instanceof LivingEntity _livingEntity33 && _livingEntity33.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity33.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0)));
									entityiterator.setDeltaMovement(new Vec3(
											(sourceentity.getLookAngle().x
							...
							if (!entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("gts", "giantessfaction")))) {
								if (entityiterator instanceof LivingEntity _livEnt42 && _livEnt42.isBlocking()) {
									if (entityiterator instanceof LivingEntity _entity) {
										...
										}, (float) (1 * (sourceentity instanceof LivingEntity _livingEntity43 && _livingEntity43.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity43.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0)));
									}
									entityiterator.setDeltaMovement(new Vec3(
											(sourceentity.getLookAngle().x
									...
									entityiterator.hurt((sourceentity instanceof LivingEntity _attacker ? _attacker.damageSources().mobAttack(_attacker) : sourceentity.damageSources().generic()),
											(float) (2.5 * (sourceentity instanceof LivingEntity _livingEntity52 && _livingEntity52.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity52.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0)));
									entityiterator.setDeltaMovement(new Vec3(
											(sourceentity.getLookAngle().x
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(sourceentity.getX(), sourceentity.getY(), sourceentity.getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("minecraft", "entity.goat.ram_impact")), SoundSource.HOSTILE,
								(float) (sourceentity instanceof LivingEntity _livingEntity64 && _livingEntity64.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity64.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0), 1);
					} else {
						_level.playLocalSound((sourceentity.getX()), (sourceentity.getY()), (sourceentity.getZ()), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("minecraft", "entity.goat.ram_impact")), SoundSource.HOSTILE,
								(float) (sourceentity instanceof LivingEntity _livingEntity64 && _livingEntity64.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity64.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0), 1, false);
					}
				}
			}
		});
		GtsMod.queueServerWork((int) 27.5, () -> {
			if (sourceentity.isAlive()) {
				sourceentity.setDeltaMovement(new Vec3(0, 0, 0));
				if (sourceentity instanceof Mob _mobSetNoAi) {

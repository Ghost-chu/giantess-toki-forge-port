...
public class TokiKickProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (sourceentity instanceof TokiEntity _datEntSetS)
			_datEntSetS.getEntityData().set(TokiEntity.DATA_AnimationDecision, "Toki.kick");
			...
			if (sourceentity.isAlive()) {
				{
					...
					List<Entity> _entfound = world.getEntitiesOfClass(Entity.class,
							new AABB(_center, _center).inflate(
									((sourceentity instanceof LivingEntity _livingEntity23 && _livingEntity23.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity23.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0) * 1.5) / 2d),
						...
						if (!entityiterator.getType().is(TagKey.create(Registries.ENTITY_TYPE, new ResourceLocation("gts", "giantessfaction")))) {
							if (entity instanceof LivingEntity _livEnt25 && _livEnt25.isBlocking()) {
								if (entityiterator instanceof LivingEntity _entity) {
									...
									_entity.hurt(new DamageSource(_dmgsource.typeHolder(), _dmgsource.getEntity(), _dmgsource.getDirectEntity()) {
										...
										public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
											String _translatekey = (entity.getDisplayName().getString() + " tried and failed to block Toki's kick, and died from embarrassment");
									...
									}, (float) (1 * (sourceentity instanceof LivingEntity _livingEntity26 && _livingEntity26.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity26.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0)));
								}
								...
								entityiterator.hurt((sourceentity instanceof LivingEntity _attacker ? _attacker.damageSources().mobAttack(_attacker) : sourceentity.damageSources().generic()),
										(float) (2.5 * (sourceentity instanceof LivingEntity _livingEntity35 && _livingEntity35.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity35.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0)));
								entityiterator.setDeltaMovement(new Vec3(
										(sourceentity.getLookAngle().x
										...
										0.5, (sourceentity.getLookAngle().z
												* (sourceentity instanceof LivingEntity _livingEntity41 && _livingEntity41.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity41.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0))));
							}
						}
					}
				}
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("minecraft", "entity.goat.ram_impact")), SoundSource.HOSTILE,
								(float) (sourceentity instanceof LivingEntity _livingEntity44 && _livingEntity44.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity44.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0), 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("minecraft", "entity.goat.ram_impact")), SoundSource.HOSTILE,
								(float) (sourceentity instanceof LivingEntity _livingEntity44 && _livingEntity44.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity44.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0), 1, false);
					}
				}
			}
		});

...
public class CandyTickUpdateProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide() && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (entity.getPersistentData().getDouble("CandyTickTimer") <= 0) {
				if (!(entity instanceof Mob _mobGetNoAi ? _mobGetNoAi.isNoAi() : false)) {
					CandyRangeAndBreachProcedure.execute(world, entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null, entity);
				}
				entity.getPersistentData().putDouble("CandyTickTimer", 3);
			} else {
				entity.getPersistentData().putDouble("CandyTickTimer", (entity.getPersistentData().getDouble("CandyTickTimer") - 1));
			}
		}
	}
}

...
public class TamedAraLowerVore2Procedure {
	public static void execute(LevelAccessor world, double x, double y, double z, Entity entity, Entity sourceentity) {
		...
		if (sourceentity instanceof TamedAraEntity _datEntSetS)
			_datEntSetS.getEntityData().set(TamedAraEntity.DATA_AnimationDecision, "Ara.LowNom2");
		...
		if (sourceentity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(GtsModMobEffects.GTS_TIMER.get(), 185, 1, false, false));
		GtsMod.queueServerWork(18, () -> {
		...
		GtsMod.queueServerWork(75, () -> {
			if ((entity.getVehicle()) == sourceentity && sourceentity.isAlive()) {
				if (entity instanceof LivingEntity _entity && !_entity.level().isClientSide())
					_entity.addEffect(new MobEffectInstance(GtsModMobEffects.GTS_SHRINK_EFFECT.get(), 110, 1));
			}
		});
		GtsMod.queueServerWork(185, () -> {
			if ((entity.getVehicle()) == sourceentity && sourceentity.isAlive() && entity.isAlive()) {
					...
					if (entity instanceof LivingEntity _entity) {
						...
						_entity.hurt(new DamageSource(_dmgsource.typeHolder(), _dmgsource.getEntity(), _dmgsource.getDirectEntity()) {
							...
							public Component getLocalizedDeathMessage(LivingEntity _msgEntity) {
								String _translatekey = (entity.getDisplayName().getString() + " became Ara's snack");
				...
				if (world instanceof Level _level) {
					if (!_level.isClientSide()) {
						_level.playSound(null, BlockPos.containing(x, y, z), BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("gts", "ara_swallow")), SoundSource.HOSTILE,
								(float) (sourceentity instanceof LivingEntity _livingEntity48 && _livingEntity48.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity48.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0), 1);
					} else {
						_level.playLocalSound(x, y, z, BuiltInRegistries.SOUND_EVENT.get(new ResourceLocation("gts", "ara_swallow")), SoundSource.HOSTILE,
								(float) (sourceentity instanceof LivingEntity _livingEntity48 && _livingEntity48.getAttributes().hasAttribute(GtsAttributes.SCALE.get()) ? _livingEntity48.getAttribute(GtsAttributes.SCALE.get()).getBaseValue() : 0), 1, false);
					}
				}
				TamedAraLowerVore2EndProcedure.execute(world, x, y, z, sourceentity);
			} else if (sourceentity.isAlive() && !((sourceentity instanceof TamedAraEntity _datEntS ? _datEntS.getEntityData().get(TamedAraEntity.DATA_AnimationDecision) : "").equals("Ara.PreyEscape")) && sourceentity.getPersistentData()

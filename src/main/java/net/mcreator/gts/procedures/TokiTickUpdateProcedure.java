...
public class TokiTickUpdateProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!world.isClientSide() && !((entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null) == null)) {
			...
			if (entity.getPersistentData().getDouble("TokiTickTimer") <= 0) {
				if (!(entity instanceof Mob _mobGetNoAi ? _mobGetNoAi.isNoAi() : false)) {
					TokiRangeAndBreachProcedure.execute(world, entity instanceof Mob _mobEnt ? (Entity) _mobEnt.getTarget() : null, entity);
				}
				entity.getPersistentData().putDouble("TokiTickTimer", 3);
			} else {
				entity.getPersistentData().putDouble("TokiTickTimer", (entity.getPersistentData().getDouble("TokiTickTimer") - 1));
			}
		}
	}
}
